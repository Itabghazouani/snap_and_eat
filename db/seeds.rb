...
# The data can then be loaded with the bin/rails db:seed command (or created alongside the database with db:setup).
  ...
  50.times do |i|
    address = addresses[i % addresses.length]  # This ensures we cycle through all addresses
    ...
    if restaurant.save
      puts "Created restaurant #{i+1}: #{restaurant.name} at #{restaurant.address}. Coordinates: #{restaurant.latitude}, #{restaurant.longitude}"
    else
      puts "Failed to create restaurant #{i+1}: #{restaurant.errors.full_messages.join(', ')}"
    end
  end
